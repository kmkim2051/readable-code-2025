...
public class MinesweeperGame {
    ...
    private static Integer[][] landMineCounts = new Integer[BOARD_HEIGHT][BOARD_WIDTH];
    private static boolean[][] landMines = new boolean[BOARD_HEIGHT][BOARD_WIDTH];
    private static int gameStatus = 0; // 0: 게임 중, 1: 승리, -1: 패배

    public static void main(String[] args) {
        printGameIntroComments();
        ...
        for (int i = 0; i < MINE_COUNT; i++) {
            int col = new Random().nextInt(BOARD_WIDTH);
            int row = new Random().nextInt(BOARD_HEIGHT);
        ...
        for (int row = 0; row < BOARD_HEIGHT; row++) {
            for (int col = 0; col < BOARD_WIDTH; col++) {
                ...
                if (!landMines[row][col]) {
                    ...
                    if (col + 1 < BOARD_WIDTH && landMines[row][col + 1]) {
                        count++;
                    }
                    if (row + 1 < 8 && col - 1 >= 0 && landMines[row + 1][col - 1]) {
                        count++;
                    }
                    if (row + 1 < BOARD_HEIGHT && landMines[row + 1][col]) {
                        count++;
                    }
                    if (row + 1 < BOARD_HEIGHT && col + 1 < BOARD_WIDTH && landMines[row + 1][col + 1]) {
                        count++;
                    }
        ...
        while (true) {
            ...
            if (userActionInput.equals("2")) {
                ...
                boolean iaAllBoardOpen = true;
                for (int row = 0; row < 8; row++) {
                    for (int col = 0; col < 10; col++) {
                        if (board[row][col].equals("□")) {
                            iaAllBoardOpen = false;
                        }
                    }
                }
                if (iaAllBoardOpen) {
    ...
    private static void printGameIntroComments() {
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
